...
mergeTS :: TS a -> TS a -> TS a
mergeTS (TS [] []) ts2 = ts2
mergeTS ts1 (TS [] []) = ts1
mergeTS (TS t1 v1) (TS t2 v2) = TS completeTimes combinedValues
...
movinAvg :: Real a => [Maybe a] -> Int -> [Maybe Double]
movinAvg [] n = []
movinAvg vals n = 
  if length nextVals == n
    then meanMaybe nextVals:movinAvg restVals n
    else []
  where 
