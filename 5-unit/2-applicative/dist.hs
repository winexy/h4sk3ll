...
haversineIO from to = do 
  coords1 <- from
  coords2 <- to
  return (haversine coords1 coords2)

from :: IO LatLong
from = return (29.9714,-90.7694)

to :: IO LatLong
to = return (42.6054,-70.7829)

haversineIOapp :: IO LatLong -> IO LatLong -> IO Double
haversineIOapp from to = haversine <$> from <*> to
